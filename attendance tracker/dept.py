...
app = Flask(__name__)
app.config['SQLALCHEMY_DATABASE_URI'] = 'postgresql://postgres:admin@localhost:5432/attendance2'
...
app.config['MAIL_USERNAME'] = 'shanmugasundar.robotics@gmail.com'
...
class Manager(db.Model):
    ...
    manager_email = db.Column(db.String(50), unique=True, nullable=False)
    manager_password = db.Column(db.String(50), nullable=False)
...
def send_leave_email(leaves, user):
    try:
        msg = Message('Leave Application', sender='shanmugasundar.robotics@gmail.com', recipients=['sundaradnim@gmail.com', user.email])
        msg.body = f"Hello request for leave is submitted successfully ,\n\nYour leave details\n\nfrom_date: {leaves.from_date}\nto_date: {leaves.to_date}\ndiscription:{leaves.discription}\nreason: {leaves.reason}\nstatus: {leaves.status}\n\nThank you\nregards\n{leaves.username}"
        mail.send(msg)
        print("Email sent successfully!")
    except Exception as e:
        print("Error sending email:", e)
...
def send_leave_rejection_email(leaves, user):
    try:
        msg = Message('Leave Application Rejection', sender='shanmugasundar.robotics@gmail.com', recipients=[user.email])
...
def register():
    ...
    try:
        ...
        if Admin.query.filter_by(name=name).first() or Admin.query.filter_by(Email=Email).first():
            return jsonify({'message': 'email already exists. Please choose different ones.'}), 400
...
def login():
    ...
    try:
        ...
        # Check if the login is for admin or user
        admin = Admin.query.filter_by(Email=email, password=password).first()
        if admin:
            # If it's an admin, create and return admin JWT token
        ...
        # If not an admin, check if it's a user login
        ...
        if user:
            ...
            if last_attendance and last_attendance.logout_time:
                # If user has previously logged out, mark status as "present"
                status = 'present'
            else:
                # If user has not logged out previously, mark status as "absent"
                status = 'absent'
            ...
            # Create and return user JWT token
            access_token = create_access_token(identity=user.user_id)
            return jsonify({"message": "User logged in successfully", "access_token": access_token}), 200

        # If neither admin nor user login succeeds
        return jsonify({'message': 'Invalid email or password'}), 401

    except Exception as e:
        return jsonify({"message": "Error logging in", "error": str(e)})
...
def add_user():
    try:
        ...
        new_user = User(username=username, email=email, password=password, Admin_id=admin.Admin_id, created_at=datetime.now())
...
def apply_leave():
    ...
    try:
        ...
        # Modify to include 'status' while creating a leave instance
        leave = Leaves(
            user_id=user_id,
            username=username,
            from_date=from_date,
...
def get_all_leaves():
    try:
        ...
        return jsonify({"message": "Error retrieving leaves", "error": str(e)}), 500
...
def get_user_leave(user_id):
    try:
        ...
        leave_data = [{'LeaveId': leave.LeaveId,'user_id':leave.user_id,'username':leave.username, 'reason': leave.reason, 'status': leave.status} for leave in user_leave]
        return jsonify({'user_leave': leave_data}), 200
    except Exception as e:
        return jsonify({"message": "Error retrieving user leave", "error":str(e)}),500
...
# TASK 2
# Leave approval and reject by admin
# @app.route('/approve_or_reject_leave/<int:LeaveId>', methods=['PUT'])
# @jwt_required()
# def approve_or_reject_leave(LeaveId):
#     try:
#         current_admin_email = get_jwt_identity()
#
...
#             return jsonify({'message': 'Admin not found'}), 404
#
#         # Retrieve the leave request to approve or reject
#         leave = Leaves.query.get(LeaveId)
#         if not leave:
...
#             return jsonify({'message': 'Invalid action. Please specify "approve" or "reject"'}), 400
#
...
# function to send mail for leave
...
def approve_leave(leave_id):
    try:
        ...
        # Retrieve the leave request to approve
        ...
        return jsonify({"message": "Error approving leave", "error": str(e)}), 500
...
def reject_leave(leave_id):
    try:
        ...
        # Retrieve the leave request to reject
        leave = Leaves.query.get(leave_id)
        if not leave:
            return jsonify({'message': 'Leave request not found'}), 404
        ...
        return jsonify({"message": "Error rejecting leave", "error": str(e)}), 500
...
def get_absent_users_data():
    try:
        ...
        # Query the database to get the count of absent users within the specified date range
        ...
        # Retrieve details of absent users
        ...
        return jsonify({"message": "Error fetching absent users data", "error": str(e)}), 500
...
def get_present_users_data():
    try:
        ...
        # Query the database to get the count of present users within the specified date range
        ...
        # Count the number of present users
        present_users_count = present_users_query.count()

        # Retrieve details of present users
        ...
        return jsonify({"message": "Error fetching present users data", "error": str(e)}), 500
...
def get_rejected_leaves(date):
    ...
    leaves = Leaves.query.filter(db.func.lower(Leaves.status) == 'rejected').filter(Leaves.from_date <= formatted_date).filter(Leaves.to_date >= formatted_date).all()
    ...
    leaves_count = Leaves.query.filter(db.func.lower(Leaves.status) == 'rejected').filter(Leaves.from_date <= formatted_date).filter(Leaves.to_date >= formatted_date).count()
...
def get_pending_leaves(date):
    ...
    formatted_date = date_obj.strftime('%d/%m/%Y')
    leaves = Leaves.query.filter(db.func.lower(Leaves.status) == 'pending').filter(Leaves.from_date <= formatted_date).filter(Leaves.to_date >= formatted_date).all()
    ...
    leaves_count = Leaves.query.filter(db.func.lower(Leaves.status) == 'pending').filter(Leaves.from_date <= formatted_date).filter(Leaves.to_date >= formatted_date).count()
...
def get_approved_leaves(date):
    ...
    formatted_date = date_obj.strftime('%d/%m/%Y')
    leaves = Leaves.query.filter(db.func.lower(Leaves.status) == 'approved').filter(Leaves.from_date <= formatted_date).filter(Leaves.to_date >= formatted_date).all()
    ...
    leaves_count = Leaves.query.filter(db.func.lower(Leaves.status) == 'approved').filter(Leaves.from_date <= formatted_date).filter(Leaves.to_date >= formatted_date).count()
...
def get_pending_leaves_data():
    try:
        ...
        # Query the database to get the count of pending leaves within the specified date range
        ...
        # Retrieve details of pending leaves
        ...
        return jsonify({"message": "Error fetching pending leaves data", "error": str(e)}), 500
...
def get_rejected_leaves_data():
    try:
        ...
        # Query the database to get the count of rejected leaves within the specified date range
        ...
        # Count the number of rejected leaves
        rejected_leaves_count = rejected_leaves_query.count()

        # Retrieve details of rejected leaves
        ...
        return jsonify({"message": "Error fetching rejected leaves data", "error": str(e)}), 500
...
def get_total_leaves_data():
    try:
        ...
        # Query the database to get the total leaves within the specified date range
        total_leaves_query = Leaves.query.filter(
            func.date(Leaves.from_date) >= start_date.date(),
            func.date(Leaves.to_date) <= end_date.date()
        )

        # Retrieve the total leaves and their count
        total_leaves = []
        ...
        return jsonify({"message": "Error fetching total leaves data", "error": str(e)}), 500
...
def get_present_count(date):
    ...
    # Query the database to get present data for the specified date
    present_data = Attendance.query.filter(
        func.date(Attendance.login_time) == formatted_date,
        Attendance.status == 'present'
    ).all()

    # Process the present data
    present_list = []
    ...
    # Count the number of present entries for the specified date
...
def get_absent_data(date):
    ...
    # Query the database to get absent data for the specified date
    absent_data = Attendance.query.filter(
        func.date(Attendance.login_time) == formatted_date,
        Attendance.status == 'absent'
    ).all()

    # Process the absent data
    absent_list = []
    ...
    # Count the number of absent entries for the specified date
...
@jwt_required()  # Assuming authentication is required
def get_absent_count_by_id(user_id):
    try:
        ...
        # Query absent count for a specific user within the specified date range
        ...
        # Query absent data for the specified user within the specified date range
        ...
        return jsonify({"message": "Error fetching absent count for user", "error": str(e)}), 500
...
def get_present_count_by_id(user_id):
    try:
        ...
        # Query present count for a specific user within the specified date range
        ...
        # Query present data for the specified user within the specified date range
        ...
        for present_entry in present_data:
            present_info = {
                ...
                'logout_time': present_entry.logout_time.strftime("%Y-%m-%d %H:%M:%S") if present_entry.logout_time else None,
        ...
        return jsonify({"message": "Error fetching present count for user", "error": str(e)}), 500
...
def get_total_leave_count_by_id(user_id):
    try:
        ...
        # Query total leave count for a specific user within the specified date range
        total_leave_count = Leaves.query.filter_by(user_id=user_id).filter(
            func.date(Leaves.from_date) >= start_date.date(),
            func.date(Leaves.to_date) <= end_date.date()
        ).count()

        # Query leave data for the specified user within the specified date range
